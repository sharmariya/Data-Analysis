...
select s.prod_id,p.product_name,sum(qty) as sold
...
select product_name, sum((qty*s.price)*(1-discount*0.01)) as Total_Revenues from sales s
join product_details pd on s.prod_id = pd.product_id
group by product_name
order by 2

--3. What was the total discount amount for all products?
select product_name, round(sum((discount*(qty*s.price)/100.0)),2) as Total_Discount
...
select category_name,sum(qty) as total_qty,round(sum((qty*s.price)*(1-discount*0.01)),2) as total_revenue,round(sum(discount*qty*s.price/100.0),2)as total_dis
from sales s
join product_details pd on s.prod_id = pd.product_id
...
--Sales Table
select s.prod_id,p.product_name,sum(qty) as sold,sum((qty*s.price)*(1-discount*0.01)) as Total_Revenues,
round(sum((discount*(qty*s.price)/100.0)),2) as Total_Discount
from sales s join product_details p 
on s.prod_id=p.product_id
group by s.prod_id,p.product_name

--Member Vs Non-Member impact on transactions and revenue
select sum(case when member='t' then 1 else 0 end)*100.0/count(*) as member_transaction,
